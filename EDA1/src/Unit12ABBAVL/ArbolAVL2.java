...
/**
 * Implementa m�todos de AbstractCollection y AbstractSet de forma espec�fica
 * para
 * una estructura de arbol AVL, mejorando en ciertos casos la velocidad de
 * ejecucion.
 *
 ...
 */
public class ArbolAVL2<T> extends java.util.AbstractSet<T> {
	/**
	 * Nodo raiz del arbol.
	 */
	private Nodo<T> raiz;
	...
	/**
	 * Constructor para recibir el comparador que se utilizar� para comparar los
	 * elementos. Si
	 * no se construye el �rbol con este m�todo los objetos se compararan en funci�n
	 * del orden
	 * natural (comparable).
	 * 
	 * @param cmp comparador
	 */
	public ArbolAVL2(Comparator<T> cmp) {
		this.comparador = cmp;
	}
			...
			while (equilibrado(this.getRaiz()) < 0) {
				...
				if (compararDato(superior.getDato(), raizTmp.getDato()) != 0) {
					if (superior.getIzquierda() != null
							&& compararDato(superior.getIzquierda().getDato(), raizTmp.getDato()) == 0) {
						superior.setIzquierda(cambiar);
					} else if (superior.getDerecha() != null
							&& compararDato(superior.getDerecha().getDato(), raizTmp.getDato()) == 0) {
	 ...
	 * Rotacion simple derecha, rotacion compuesta derecha izquierda, rotacion
	 * simple
	...
	public boolean addAll(Collection<? extends T> c)
			throws ClassCastException, NullPointerException, IllegalStateException {
		Iterator<? extends T> iter = c.iterator();
		Iterator<? extends T> iter2 = c.iterator();
		T dato, primero;
		boolean insertado = false;

		// si el arbol no exist�a
		if (this.isEmpty()) {
		...
		// el arbol ya exist�a
			...
			// comprobamos que los datos sean comparables con los que ya est�n dentro del
			// arbol
		...
		// solo llega hasta aqui si los elementos son comparables entre si o con los que
		// ya hab�a
		while (iter2.hasNext()) {
			dato = iter2.next();
			if (dato != null) {
	...
	public boolean remove(Object o) throws ClassCastException, NullPointerException {
		...
		if (salir) {
			...
			if (borrar.getIzquierda() == null && borrar.getDerecha() == null) {
				...
				if (nPadre.getIzquierda() != null
						&& compararDato(nPadre.getIzquierda().getDato(), borrar.getDato()) == 0) {
					nPadre.setIzquierda(null);
				} else if (nPadre.getDerecha() != null
						&& compararDato(nPadre.getDerecha().getDato(), borrar.getDato()) == 0) {
			...
			// subarbol ser� 1 o 2 (altura raiz = 1)
			...
			// cuando no es ni un hoja ni su padre. Es decir, est� por medio del arbol.
				...
				// si no tiene hijos ya, lo eliminamos sin m�s
					...
					if (papa.getIzquierda() != null
							&& compararDato(papa.getIzquierda().getDato(), cambiar.getDato()) == 0) {
			...
			while (equilibrado(this.getRaiz()) < 0) {
				...
				if (compararDato(superior.getDato(), mirar.getDato()) != 0) {
					if (superior.getIzquierda() != null
							&& compararDato(superior.getIzquierda().getDato(), mirar.getDato()) == 0) {
						superior.setIzquierda(cambiar2);
					} else if (superior.getDerecha() != null
							&& compararDato(superior.getDerecha().getDato(), mirar.getDato()) == 0) {
	 ...
	 * @return c - collection containing elements to be removed from this
	 *         collection.
	 ...
	 *                              elements and the specified collection does not
	 *                              support null elements (optional), or if the
	 *                              specified collection is null.
	 */
	...
	/**
	 * Este m�todo recorre el �rbol mediante el recorrido INORDEN y almacena cada
	 * elemento.
	 ...
	 * el dato nose encuentra en el �rbol se devuelve -1.
	 * 
	...
	/**
	 * Se devuelve la profundidad del Nodo que contiene al dato pasado por
	 * par�metro,
	 * si eldato no se encuentra en el �rbol se devuelve -1. La profundidad del nodo
	 * ra�z es 0.
	 * 
	...
	/**
	 * Devuelve el Nodo que contiene al dato pasado por par�metro.
	 * 
	...
	/**
	 * Devuelve el comparator que utiliza el �rbol.
	 * 
	 ...
	 *         menor, igual o mayor respectivametne con especto al segundo objeto
